...
exports.updateGstValue = catchAsyncErrors(async (req, res) => {
  ...
  const isSettingPresent = await Setting.findOne();

  let setting;
  if (!isSettingPresent) {
    setting = await Setting.create({ gstValue });
  }

  setting = await Setting.findOneAndUpdate({ gstValue });

  res
    .status(200)
    .json(new ApiResponse(200, setting, "Gst Value updated successfully"));
});
...
exports.getGstValue = catchAsyncErrors(async (req, res) => {
  ...
  res
    .status(200)
    .json(
      new ApiResponse(200, setting.gstValue, "Gst Value updated successfully")
    );
});
...
exports.updateDayRate = catchAsyncErrors(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, {}, "Day Rate updated successfully"));
});
...
exports.sendPdf = catchAsyncErrors(async (req, res) => {
  ...
  const subtotal = invoices.reduce(
    (acc, invoice) => acc + invoice.totalDayAmount,
    0
  );
  ...
  const mailSender = new MailSender(
    email,
    "Bill Details 💵💸",
    "Attached is your Bill Details For the month of Febuary 2024.",
    emailBody
  );
...
exports.updateKmRate = catchAsyncErrors(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, {}, "Km rate updated successfully"));
});
