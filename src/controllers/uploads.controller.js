...
function generateTripId(district, frvCode) {
  ...
  const frvCodePrefix = frvCode
    .substring(randomIndex, randomIndex + 3)
    .toUpperCase();
...
exports.addMultipleOwner = catchAsyncErrors(async (req, res) => {
  ...
  for (const owner of owners) {
    ...
    if (isOwnerExists) {
      console.log("owner already exists and owner is as follows", owner);
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {}, "Owners Added Successfully"));
});
...
exports.addCars = catchAsyncErrors(async (req, res) => {
  ...
  for (const car of cars) {
    const owner = await Owner.findOne({
      "registration.registrationNo": car.registrationNo,
    });
...
exports.handleTripsInvoices = catchAsyncErrors(async (req, res) => {
  ...
  for (const registrationNo in data.carStats) {
    if (Object.hasOwnProperty.call(data.carStats, registrationNo)) {
      ...
      for (const trip of carStat.trips) {
        ...
        const isTripPresent = await Trip.findOne({
          car: car._id,
          company: company._id,
  ...
  for (const registrationNo in data.carStats) {
    if (Object.hasOwnProperty.call(data.carStats, registrationNo)) {
      ...
      for (const trip of carStat.trips) {
        ...
        invoice.trip = prevTrip._id.toString();
        invoice.dayQty += Number(trip.totalDays) || 0;
        invoice.kmQty += Number(trip.totalKm) || 0;
        invoice.offroad += trip.offroad || 0;
        invoice.dayAmount +=
          Number((trip.totalDays * trip.rate.date).toFixed(2)) || 0;
        invoice.kmAmount +=
          Number((trip.totalKm * trip.rate.km).toFixed(2)) || 0;
        invoice.totalAmount +=
          Number(
            (
              trip.totalDays * trip.rate.date +
              trip.totalKm * trip.rate.km
            ).toFixed(2)
          ) || 0;
        invoice.gstAmount = Number(
          ((invoice.totalAmount * setting.gstValue) / 100).toFixed(2)
        );
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {}, "data uploaded Successfully"));
});
