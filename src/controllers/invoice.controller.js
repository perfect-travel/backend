...
exports.generateInvoice = catchAsyncErrors(async (req, res) => {
	...
	if (isInvoicePresent) {
		console.log(isInvoicePresent);
		console.log(trip);
		isInvoicePresent.dayQty += dayqty;
		isInvoicePresent.kmQty += kmqty;
		isInvoicePresent.offroad += trip.months[trip.months.length - 1].offroad;
		isInvoicePresent.dayAmount += dayAmount;
		isInvoicePresent.kmAmount += kmAmount;
		isInvoicePresent.totalAmount += total;
		isInvoicePresent.gstAmount += gstTotal;
		isInvoicePresent.billAmount += billTotal;

		isInvoicePresent.months.push({
			startDate: trip?.start?.date,
			endDate: end?.date,
			startKm: trip?.start?.km,
			endKm: end?.km,
			offroad: isInvoicePresent?.offroad,
			invoiceDate: end.date || new Date(),
			days: isInvoicePresent.dayQty + isInvoicePresent?.offroad,
			rate: {
				date: trip?.car?.rate?.date,
				km: trip?.car?.rate?.km,
			},
			rent: car.rent,
			district: trip.district,
			frvCode: trip.frvCode,
			totalDays: isInvoicePresent.dayQty,
			dayAmount: dayAmount,
			kmAmount: kmAmount,
			totalAmount: total,
			gstAmount: Number(gstTotal),
		...
		await isInvoicePresent.save();
		await car.save();
		await trip.save();

		res.status(201).json(new ApiResponse(200, {}, "Invoice generated successfully."));
	} else {
